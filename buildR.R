...
PathIS=(dirname(rstudioapi::getActiveDocumentContext()$path))
PathIS_data = paste(PathIS,'/', sep = "")
setwd(PathIS)
# setwd('/Users/zwdu/Documents/Github/sir')
...
hh=read.csv("Csv/perpub.csv",as.is=T)
...
zz1=tapply(hh1$HHSIZE, hh1$HOUSEID, mean) # houseid and house size
zz2=tapply(hh1$URBAN, hh1$HOUSEID, mean) # houseURBAN and house size
...
b = read.csv("Austin Population - 5 age groups.csv")
pp=colSums(b)[-(1:2)]

pop = pp
x=write.net( pop,a,file="AustinFullNet.csv",append=F)
x=write.net( floor(pop/10),a,file="pholme_sir/AustinSmallNet6.csv",append=F)
x=write.net( floor(pop/10000),a,file="pholme_sir/AustinTinyNet6.csv",append=F) 
...
library(pryr)
groups=list(a=1:10,b=11:20,c=21:30,d=31:40,e=41:50)
p=sapply(groups,length)
...
# Just a bit about how to run the sim.
...
# which means load the network from AustinSmall10Net10.csv
# set random seed to 63 (I think this is actually ignored. I need to change that)
# beta=0.03
# first time section, day 0 to 50, set weights 1=1,2=1,3=1, then period 50-200 set weights 1=0.25,2=1,3=0
# Any weight indexes that appear in the network file and aren't specified in a time period are assumed to be 1. (edited) 
# Sorry about the strange way to specify beta, it comes from a previous iteration where you specified more parameters